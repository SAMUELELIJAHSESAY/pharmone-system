...
export async function updatePatientVisit(visitId, payload) {
  const { data, error } = await supabase.from('patient_visits').update(payload).eq('id', visitId).select().single();
  if (error) throw error;
  return data;
}
...
export async function updateTreatmentPayment(paymentId, payload) {
  const { data, error } = await supabase.from('treatment_payments').update(payload).eq('id', paymentId).select().single();
  if (error) throw error;
  return data;
}
